...
//! - **WASM**: [`ShareUrlRestoredWorkspace`] が真なら `Editing`。それ以外は [`AppScreen::PresetPicker`](super::routing::AppScreen::PresetPicker)。
...
#[derive(Resource, Debug, Clone, Copy, Default)]
pub(crate) struct ShareUrlRestoredWorkspace(pub bool);
...
pub(crate) fn resolve_initial_app_screen(
    mut next: ResMut<NextState<AppScreen>>,
    url: Res<ShareUrlRestoredWorkspace>,
) {
    if url.0 {
        next.set(AppScreen::Editing);
    }
